...
| `resection_geometry.focal_absorbed_radial` | The **third** nuisance parameter: an unknown pixels-per-degree. A crop, screenshot or stock photograph has lost its scale, and an unknown scale is degenerate with radial position — moving toward the terrain magnifies everything, which is what a shorter focal length does. Measured on the Lake Tahoe wallpaper: with the scale free, moving the camera **8 km** changed the residual by 0.1 arcmin. `position_dilution(..., focal_free=True)` reports the honest ellipse. |
