...
| `related_work.md` | **Prior art**: terrain-referenced nav (TERCOM/TRN), bathymetric graph SLAM, skyline geo-localisation and DEM-anchored SLAM — with **licences verified per repo**, what is safe to reuse, and what is deliberately not. Notes the gap: no public repo fuses a photographed horizon against SRTM *in a factor graph*. |
