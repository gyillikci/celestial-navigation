...
final class PipelineTests: XCTestCase {
    ...
    // MARK: apparent-altitude corrections vs the Python golden vectors

    func testCorrectionsMatchGoldenVectors() {
        // GoldenVectors.swift is generated by imu_fusion/export_golden.py from the
        // validated pipeline (cross-checked vs ERFA/astropy to sub-arcminute).
        XCTAssertGreaterThanOrEqual(goldenVectors.count, 4)
        for g in goldenVectors {
            // Forward: geometric → apparent must reproduce the reference to ~1e-4°.
            let app = CelestialMath.apparentFromGeometric(
                geocentricAltDeg: g.geometricAltDeg, distanceKm: g.distanceKm)
            XCTAssertEqual(app, g.apparentAltDeg, accuracy: 1e-4,
                           "\(g.body) apparent @ \(g.utc)")
            // Inverse (the device path): apparent → geometric round-trips exactly.
            let geo = CelestialMath.geometricFromApparent(
                apparentAltDeg: g.apparentAltDeg, distanceKm: g.distanceKm)
            XCTAssertEqual(geo, g.geometricAltDeg, accuracy: 1e-4)
            // The published pieces agree too.
            XCTAssertEqual(CelestialMath.horizontalParallaxDeg(distanceKm: g.distanceKm),
                           g.hpDeg, accuracy: 1e-4)
            XCTAssertEqual(CelestialMath.parallaxInAltitudeDeg(
                            geometricAltDeg: g.geometricAltDeg, distanceKm: g.distanceKm),
                           g.parallaxDeg, accuracy: 1e-4)
        }
    }

    func testMoonParallaxDominatesLowSunNegligible() {
        // Moon low in the sky: apparent is well BELOW geometric (parallax wins).
        let moon = CelestialMath.apparentFromGeometric(geocentricAltDeg: 15,
                                                       distanceKm: 384_400)
        XCTAssertLessThan(moon - 15, -0.7)
        // Sun: parallax negligible, small positive refraction lift.
        let sun = CelestialMath.apparentFromGeometric(geocentricAltDeg: 15,
                                                      distanceKm: 1.495978707e8)
        XCTAssertTrue(sun - 15 > 0 && sun - 15 < 0.1)
    }
